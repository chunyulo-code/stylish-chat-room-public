...
export default function SidebarUser({ picture, name, chatRoomId }) {
  const { currentRoomId, setCurrentRoomId } = useContext(ChatAdminContext);
  ...
  return (
    <div
      className={`${
        chatRoomId === currentRoomId ? "bg-gray-200" : ""
      } flex h-[65px] cursor-pointer items-center border-b border-solid border-chatroom-gray px-[30px] py-[10px] hover:bg-gray-100`}
      onClick={() => {
